...
namespace MicroControllerDrivers
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // INITIALIZE OBJECTS TO DRIVER CLASSES
            var Interrupthandler = new InterruptHandling();
            var MemoryManagementhandler = new MemoryManagement();
            var I2CBushandler = new I2CBus();
            var InputOutputEthernetControllerhandler = new InputOutputEthernetController();
            var deviceDrivers = new List<DeviceDriverFunctions>() { Interrupthandler, MemoryManagementhandler, I2CBushandler, InputOutputEthernetControllerhandler };

            // EXECUTE PROGRAM
            Console.WriteLine("\n EXECUTE INITIAL START UP ROUTINE FOR EACH DEVICE DRIVER \n ");

            foreach (var element in deviceDrivers)
            {
                element.HardwareStartup();
                element.HardwareInstall();
                element.HardwareMapping();
                element.HardwareEnable();

                if (element is InterruptHandling)
                {
                    var newelement = (InterruptHandling)element;
                    newelement.ExternalInterrupt();
                }
            ...
            foreach (var element in deviceDrivers)
            {
                element.HardwareAcquire();
                element.HardwareRead();
                element.HardwareWrite();
                element.HardwareRelease();
            ...
            foreach (var element in deviceDrivers)
            {
                element.HardwareUnmapping();
                element.HardwareDisable();
                element.HardwareShutdown();
                Console.WriteLine("\n");
